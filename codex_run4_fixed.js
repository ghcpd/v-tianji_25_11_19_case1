...
function subscribe(fn) {
  ...
  return () => {
    ...
    if (idx !== -1) GlobalState.listeners.splice(idx, 1);
  };
}
...
// Avoid double-await, replace items (mutate array for reference stability)
async function refresh() {
  ...
  GlobalState.items.splice(0, GlobalState.items.length, ...newItems);
  notify();
}
