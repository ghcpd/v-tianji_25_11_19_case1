...
// Deep clone (avoid shallow copy bugs)
function clone(obj) {
...
// Fix race & accumulation; sort based on pre-fetch state
...
function expensiveCompute(v) {
  let x = 0;
  for (let i = 0; i < 50000; i++) x += (v * i) % 7;
  return x;
}

subscribe(render);

// Await loadItems to avoid timing bug
